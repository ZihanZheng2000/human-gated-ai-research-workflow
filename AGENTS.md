...
- **Config A** → your role is **Researcher**. Follow the [Researcher](#role-researcher-config-a) section.
- **Config B** → your role is **Reviewer**. Follow the [Reviewer](#role-reviewer-config-b) section.

---

## Role: Researcher (Config A)

This repository is a workflow kit for AI-native academic research. Treat the
workflow documents as the source of truth and run the project through explicit
stage packages and gates. The reviewer agent (Claude Code) critiques each stage
package before the user gate; you read the critique, triage findings, and then
ask the user for the gate decision.
...
When the stage package is ready for Claude Code to review, write:
...
## Role: Reviewer (Config B)

This repository is a workflow kit for AI-native academic research. Your role is
**reviewer agent**. The researcher agent (Claude Code) runs the four-stage
workflow and produces stage packages. You critique each stage package and return
structured findings for the researcher agent to triage.
...
When Claude Code is ready for your critique, it writes:
...
researcher agent (Claude Code) reads your findings, triages them, updates the
stage package, and asks the user for the gate decision.
